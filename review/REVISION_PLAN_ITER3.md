...
- 2026-09-13: P0 executed on branch `iter3/P0` (commits 10a9db3, c66b409) for issue #38; every P0 item addressed except the Fig 8 keep/trim/drop decision (#49), where the composite was kept and made legible pending the decision. PDF rebuilt: 83 pages, no `??`, no `,%`, no `<<`; the Acknowledgements placeholder remains for P6.
- 2026-09-13: P1 executed on branch `iter3/P1` (commits 6613570, 7b67e53, f62e01e) for issue #39; step-error metric taken as post- minus pre-event medians pending #50, the Yuan et al. (2021) regime left for #47. Two findings beyond the plan: the correlation-length fit was wrong (39 d from an intercept fit over 40 lags; 5.7 d from the short-lag decay; whitening check added) and the credible band on mu is about 80% prior-set (duplication check added). The shared_source and clock_drift locked reruns were still running when the PR opened and follow in the same PR.
