...
profile is where reporting thins out: many studies read one band as one depth,
only a few invert several bands against sensitivity kernels, and the measurement
error is rarely propagated into the depth estimate. Because the depth *is* the
claim — the change is in the aquifer, not the soil (Rubinstein & Beroza 2005) — a
depth reached without its uncertainty cannot support it.
...
## Partial saturation: $\delta V_S/V_S$ is not the whole story
...
(Gassmann 1951; Mavko et al. 2009) — is what breaks the degeneracy, at the price
...
(Clements & Denolle 2018; James et al. 2019), are exactly where ignoring the
