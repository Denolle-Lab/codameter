...
community ultimately wants, and the reason the measurement covariance is
worth getting right.
...
The literature agrees on the ingredients of a good \dvv~measurement ---
a stretching-family estimator for robustness at low SNR and large change
\citep{Yuan2021, Mikesell2015}, a coherence-based error model
\citep{Clarke2011, Weaver2011}, a long stable reference
\citep{Wang2017}, and cross-checking against a second estimator
\citep{Obermann2019} --- and yet no two studies report the same set, and
the uncertainty convention is the least standardized of all
(Appendix\textasciitilde{}\ref{app:survey}). The results below take each
ingredient in turn and measure, against a known truth, how far a
defensible deviation moves the recovered value \emph{and its stated
error}. The object the exercise produces is a covariance, and everything
downstream in the arc
...
The rest of the paper is a framework, not new synthetics: it sets out
how the covariance of Section\textasciitilde{}\ref{sec:bayes} propagates
down the inference chain, and grounds each step in what the monitoring
literature does and does not yet do. The executable stages are under
...
measurement as the way to resolve it
\citep{Takano2017, Feng2020, Mao2022}. The step from a set of per-band
\dvv~series to a depth profile is where reporting thins out: many
studies read a single band as a single depth, only a few invert several
bands against surface-wave sensitivity kernels, and the propagation of
the measurement error into the depth estimate is rarely shown. This
matters because the depth is the claim --- the change is in the aquifer
and not the soil, the softening is a shallow site response and not slip
on the fault at depth \citep{Rubinstein2005} --- and a depth reached
without its uncertainty cannot support it.
...
the price of its own prior uncertainty on porosity, fluid modulus, and
the saturation path. Hydrological and cryospheric targets, where
saturation swings are large \citep{Clements2018, James2019}, are exactly
where ignoring the density term biases the inferred velocity change and,
downstream, the stress. The framework carries \(\delta\rho/\rho\) as a
second inverted field with its own kernel and reports how much of the
surface \dvv~each explains; the executable version is a documented
extension (Section\textasciitilde{}\ref{sec:discussion}).
...
The reason to want \(\delta V_S/V_S(z)\) is almost never the velocity
itself; it is the stress, strain, or pore-pressure change that drives it
and --- increasingly --- \emph{which} forcing is responsible. Here the
literature is more fragmented than converged. Volcano and fault studies
often read the velocity change qualitatively (a pre-eruptive
pressurization, a coseismic damage); hydrological studies fit an
explicit poroelastic or thermoelastic model and separate the two by
their seasonal phase lag \citep{Tsai2011, Clements2018, Wang2017}; and a
growing set quantifies stress sensitivity through an acoustoelastic
coefficient \citep{Brenguier2008, Nakata2011}. What is missing almost
everywhere is the \emph{uncertainty} on that final number and an honest
apportionment of it among the competing forcings. Because the material
properties entering the conversion are poorly known, the stress
uncertainty is dominated by \emph{epistemic} prior uncertainty rather
than by the measurement --- so a stress value reported without it is the
least trustworthy number in the chain, and the one most often quoted as
a result.
...
Beyond the state of stress, the community wants attribution: how much of
...
fingerprinting scaled by the very measurement covariance \(C_d\) this
paper builds. Each forcing \(k\) predicts a depth--time effective-stress
...
of the stress variability at each depth, and the cross-covariances say
how ambiguous the attribution is when two forcings are collinear
(thermoelastic and hydrologic are both seasonal). When forcings couple,
the additive decomposition gives way to Sobol variance indices whose
interaction term \emph{is} the coupling contribution. Sampling the joint
posterior --- forcing amplitudes, material priors, and the measurement
realization --- turns each share into a credible interval and,
decomposed once more, says whether a fuzzy attribution is data-limited
or model-limited, which is to say what to measure next.
