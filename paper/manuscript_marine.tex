...
structural, not a noise effect that more averaging fixes. That
structural erasure is the reason to read the moving reference here as a
limit case rather than as common practice: none of the 103 studies in
our survey (Appendix\textasciitilde{}\ref{app:survey}) reports
re-baselining against a trailing reference at all. The schemes coded as
``moving'\,' in the survey are moving \emph{current} stacks read against
a fixed long-term reference --- \citet{Brenguier2008} measure
overlapping 5-day windows against a 1550-day reference, \citet{Wang2017}
a moving-window current stack against a \$\sim\$1-yr reference --- which
is the substacking axis of
Section\textasciitilde{}\ref{sec:param_substack}, not the reference
axis. The moving reference is therefore best understood as a controlled
demonstration of what continuous re-baselining costs, and the case for
the joint inversion rests on that cost directly rather than on any
analogy to a published reference construction.

Where the reference is genuinely hard to define, the surveyed responses
take a different form: \citet{Ermert2023} stabilise urban single-station
autocorrelations in Mexico City by clustering the correlation waveforms
with a Gaussian mixture model before stacking, so that the reference is
built from a coherent subset of the record rather than from a contiguous
window of it. Reference \emph{selection} of that kind is a distinct axis
from reference \emph{length}, and codameter does not currently implement
it.
