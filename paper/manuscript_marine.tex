...
for \(\tau^2\), \(s^2\) and \(\lambda\) on average; \(\lambda\) exceeds
\(1.05\) in 22 of the 200 realisations (largest \(1.19\)), and excluding
those realisations changes no coverage by more than \(0.002\). The
\(95\,\%\) credible band on \(\mu\) contains the truth at only 0.59 of
the epochs: the ensemble mean inherits the biases its members share (the
stacking that rounds the drop, the estimator biases at low coherence),
and no amount of averaging over configurations removes them, which is
what the posterior's precision cannot know. Comparing \(\mu\) with
