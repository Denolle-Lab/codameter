...
(standard deviation) of a recovered \dvv~estimate; subscripts
distinguish specific contexts and types, defined where they first
appear.
...
\(+\) methodological variance (where \(\sigma_k(t)\) is the
per-component measurement uncertainty), and \(R_{ij}=e^{-|t_i-t_j|/L}\)
for a correlation length \(L\) estimated from the ensemble residuals.
This \(C_d\) is \textbf{time-dependent} --- wider at the sharp drop and
at low coherence --- and its temporal correlation plus common-mode term
...
the Introduction. The single-measurement error on the raw stretch factor
\(\varepsilon\) (denoted \(\sigma_{\varepsilon}\)) decreases with the
