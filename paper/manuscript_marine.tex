...
random-phase wavefield modulated by a coda envelope taken from radiative
transfer. The envelope is the closed-form solution of the
two-dimensional radiative transfer equation for isotropic scattering
\citep{Sato1993, Paasschens1997}, which sums all scattering orders and
underlies coda-envelope modelling of scattering and intrinsic
attenuation \citep{Margerin1998}; the synthetic is not a wavefield
simulation, only random phases under that envelope. The energy density
at source--receiver distance \(r\) and lapse time \(t\) is
...
every departure of a recovered series from it is due to the processing
or to the additive noise at the prescribed signal-to-noise ratio, never
to an unknown Earth; the noise-limited part is what the coherence floor
of Section~\ref{sec:bayes} describes, and the rest is the processing.
...
from can cost as much as an uncumulated trailing reference, the ablation
of re-baselining without trend reconstruction
(\(\sim\!0.15\)--\(0.16\,\%\) RMS either way); a reference from the
quietest available period, of any length, recovers the trend to
\(\sim\!0.03\,\%\). A published moving-reference workflow that cumulates
or stitches its increments is not priced here.
...
never gets the chance to show up. We measure the recovered step as the
median of the 120 days after the event minus the median of the 120 days
before, computed the same way on the recovered series and on the truth
(where it is \(-0.26\,\%\)); a minimum over the post-event window, the
definition used in an earlier draft, picks the noisiest epoch and is
biased towards a deeper drop as the series gets noisier. With the median
definition the recovered step is shallower than the truth at every stack
length and for both deployments, and the under-estimate grows with the
stack length: for the workable deployment from \(0.002\,\%\) at 2 days
to \(0.014\,\%\) at 60 days, for the poor deployment from \(0.005\) to
\(0.066\,\%\), both small against the step itself. The smearing that a
longer stack applies to a step is therefore visible in the step
amplitude as well as in the RMS, and the direction of the error is
towards under-estimating the drop.
...
carries the change recovers it (Fig.~\ref{fig:branches}a). When instead
both branches share the \emph{same} change, their difference is
measurement noise, and selecting the branch of greatest change
over-reports it --- a max-of-two-estimators selection bias that grows as
SNR falls (Fig.~\ref{fig:branches}b). Choosing the branch of greatest
change after seeing the data is therefore a selection rule that
...
coherence reduces but does not remove that bias. The rule we recommend
is fixed before the data are seen: measure each branch, report both,
...
as an explicit term of the measurement covariance rather than discarding
it by averaging. The pipeline used in the rest of this paper does not
yet implement that rule: its stretching estimator fits one stretch
factor jointly over the two branches of the coda window (the default
\texttt{branch="both"} of the implementation), which is neither a
per-branch measurement nor a selection, and the covariance \(C_d\) of
Section~\ref{sec:bayes} carries no branch term. Fig.~\ref{fig:branches}
is therefore an argument for the rule, not a demonstration of it, and
the per-branch measurement is deferred.
...
(10--30\(\,\)s), and daily correlations over 2.5 years at a per-day
correlation-coefficient SNR of 7, typical of a continuously operating
station; every pipeline stacks and references on the daily grid, and
only every third day of its output is retained for the figures and the
statistics. The synthetic ground truth combines an annual seasonal
...
covered in Sections~\ref{sec:aggregation}--\ref{sec:uncertainty}.
...
the resulting error against the known truth on three paired noise
realisations, on the epochs that every configuration produced
(Fig.~\ref{fig:deviations}; Table~\ref{tab:estimands}). The ranking is
unambiguous and stable across realisations: relative to a best-practice
RMS error of \(0.033\,\%\) (\(0.031\) to \(0.035\) over the three
realisations), the two-dimensionally-unwrapped WCS estimator is
catastrophic here (\(46\times\) worse), a wrapped-phase MWCS inflates
the error by an order of magnitude (\(12\times\)), and DTW by
\(6\times\); among the non-estimator choices the uncumulated trailing
reference is worst (\(5\times\), with the largest step error; it is an
ablation of re-baselining without trend reconstruction, not a surveyed
workflow, Section~\ref{sec:param_ref}, and it produces \(96\,\%\) of the
epochs, the rest being its warm-up), while stack length, coda window and
frequency band deviations change the error by factors between \(0.7\)
and \(2.3\). Two of them score below the baseline: the \(0.8\)--\(2\) Hz
band (\(0.023\,\%\)) and the \(4\)--\(14\) s window (\(0.027\,\%\)), so
the best-practice baseline is not the optimum of this scenario. Turning
the coherence gate off leaves the error unchanged, because no epoch of
this scenario falls below the gate. The joint-inversion reference, run
with 10-day blocks to match the stack length, sits at \(1.7\times\) the
baseline; it has no per-epoch coherence, so the gate does not apply to
it, and the estimator axis does not combine with it because the
inversion is itself a stretching measurement between block stacks.
...
We test the compounding effects of these choices through the full
factorial of three estimators, two frequency bands, three coda windows,
three stacking lengths and two reference schemes, 108 workflows, on a
synthetic ``volcano'' dv/v time series that includes a seasonal
oscillation, a slow pre-eruptive inflation ramp, and a sharp co-eruptive
drop with partial exponential recovery (Fig.~\ref{fig:multiverse}a).
Twelve of the 108 return no epoch at all: MWCS with the 4--14 s coda
window, at every stack length and reference, because that window holds
two of the 6-s sub-windows and the delay-versus-lapse fit needs at least
three. The statistics below are over the 96 pipelines that return a
series. Their per-epoch standard deviation varies by a factor of
\(\sim\!4\) over the time series (from \(0.39\) to \(1.72\,\%\)), and is
widest exactly at the sharp co-eruptive drop; the RMS error against the
known truth spans more than two orders of magnitude across pipelines
(\(0.020\) to \(2.84\,\%\)).
...
(index \(0.27\)), with the estimator (\(0.19\)) and the stack length
(\(0.17\)) next and the band and the reference scheme below \(0.03\).
For the recovered step, the reference scheme leads (\(0.08\)) with the
estimator (\(0.05\)) and the window (\(0.03\)) next; the stack length
has no first-order effect on the step as measured by post- and pre-event
medians, which the smearing of a stack shorter than the 120-day windows
barely moves. The first-order indices sum to \(0.65\) for the RMS and to
\(0.17\) for the step, so a large part of the spread, and most of it for
the step, is \emph{interaction} between choices compounding. The
appropriate object is therefore not a single curve but a distribution of
\dvv~over the processing choices.
...
likelihood. One consequence can be measured directly: duplicating every
member of the ensemble, which the working likelihood counts as new data,
shrinks the median half-width of the \(95\,\%\) credible band on \(\mu\)
on the realisation of Fig.~\ref{fig:bayes} from \(0.063\) to
\(0.052\,\%\) (a factor 0.83, where independent data would give 0.71),
and quadrupling it to \(0.040\,\%\) (0.64 against 0.50), because the
smoothness prior with its fitted \(\lambda\) supplies about \(80\,\%\)
of the per-epoch precision of \(\mu\) at the posterior mean; the
single-member scale \(\sigma_{C_d}\) moves by less than \(4\,\%\) under
the same duplication (\texttt{scripts/check\_ensemble\_duplication.py}).
The credible band on \(\mu\) is thus set more by the assumed smoothness
than by the members, which is one reason its coverage has to be tested
rather than assumed. We use a second-difference smoothness prior of
precision \(\lambda\) on the latent true series \(\mu(t)\), built on the
physical time grid so that a gap in the record is a gap in the prior.
Here \(\beta_k\) is configuration \(k\)'s constant offset (the
systematic offset between, say, MWCS and stretching), \(\tau\) its scale
across the ensemble, and \(s\) rescales the coherence floor of
eq.~\ref{eq:weaver} so that the data report whether that floor is
calibrated. Every configuration is run through the same pipeline code as
the rest of the paper, so the estimator, band, window, stack length in
days, reference scheme and coherence gate all take effect; an epoch a
configuration does not produce (reference warm-up, a gated coherence) is
simply missing and carries no information; so is an epoch whose peak
stretching coherence falls below 0.5, where the floor of
eq.~\ref{eq:weaver} is undefined. The hyper-priors are conjugate:
\(\tau^2\) and \(s^2\) follow inverse-gamma distributions with shape 2
and scale \(10^{-8}\), and the smoothness precision \(\lambda\) a gamma
distribution with shape 2 and rate \(10^{-10}\); the scales are small
against the data terms, and codameter reports the prior scale term
divided by the conditional posterior rate, evaluated at posterior means.
The ratio is about 0.09 for \(\tau^2\) in the clean scenario and below
0.01 for \(s^2\) and \(\lambda\) (Table~\ref{tab:calibration}). This
diagnostic does not measure the total influence of the prior. A
...
returns the joint posterior from one chain of 1200 sweeps, the first 400
discarded and every second retained thereafter; the sampler seed is the
realisation seed, and the calibration runs below add a second chain from
another seed to check convergence.
...
\(R_{ij}=e^{-|t_i-t_j|/L}\) for a correlation length \(L\) fitted
through the origin to the leading lags of the residual autocorrelation,
those above 0.1. \(C_d\) is constructed from the fitted model, not
sampled. It targets the error of one measurement drawn from the
ensemble, not of \(\mu\). The between-configuration offset scale
...
\(R\) or the rank-one term. What can be checked is what \(R\) leaves
behind: whitening each member's residual about \(\mu\) by the Cholesky
factor of \(C_d\) on its observed epochs
(\texttt{scripts/check\_cd\_whitening.py}) should give white residuals
of unit variance. On the realisation of Fig.~\ref{fig:bayes} the raw
...
days; with \(L=5.7\) days the whitened residuals retain a lag-one
correlation of 0.26 and have a variance of 0.62, so the single
exponential captures most but not all of the stacking-induced
correlation, and the residual scatter about \(\mu\) is below the
single-member scale (which is what the \(68\,\%\) overcoverage below
also says). An earlier fit of \(L\) over 40 lags with an intercept
returned 39 days on this realisation, several times the short-lag decay,
and its whitened residuals had a variance of 3 with negative correlation
at two and three epochs; that fit is withdrawn. A downstream inversion
needs the error covariance of its actual input estimate. Neither using
this \(C_d\) for the ensemble mean nor interpreting its time axes as
frequency-band axes is justified here.
...
they usually do (Fig.~\ref{fig:bayes}c). The fitted temporal correlation
(\(L=5.7\) days on the realisation shown, \(6.1\) days on average over
the 200 realisations of Table~\ref{tab:calibration}, at a 4-day cadence)
and, above all, the common-mode term \(\tau\) collapse the effective
number of independent epochs from 229 to 29 on the realisation shown and
45 on average.
...
calibrate both interval levels. That coverage is also in-sample: \(s\)
and the excess spread are fitted on the members they score. Fitting
\(C_d\) on a random half of the twelve members and scoring the other
half gives \(0.937\pm0.004\) at the \(95\,\%\) level, still within the
margin, and \(0.79\) at \(68\,\%\). Two Gibbs chains from different
seeds give split \(\hat R\) values of \(1.006\), \(1.002\) and \(1.022\)
for \(\tau^2\), \(s^2\) and \(\lambda\) on average on the clean
scenario; \(\lambda\) exceeds \(1.05\) in 22, 40 and 27 of the 200
realisations of the clean, shared-source and clock-drift scenarios
(largest \(1.19\), \(1.90\) and \(1.81\)), and excluding those
realisations changes no coverage of Table~\ref{tab:calibration} by more
than \(0.004\). The \(95\,\%\) credible band on \(\mu\) contains the
truth at only 0.59 of the epochs: the ensemble mean inherits the biases
its members share (the stacking that rounds the drop, the estimator
biases at low coherence), and no amount of averaging over configurations
removes them, which is what the posterior's precision cannot know.
Comparing \(\mu\) with \(\pm 2\sigma_{C_d}\) mixes the ensemble mean
with a single-member scale and yields nearly complete coverage here; it
does not validate the uncertainty of the combined estimate. The other
two scenarios of Table~\ref{tab:calibration} inject artefacts that every
configuration sees identically. A station clock drift changes no
coverage by more than 0.015 and adds no shared bias
(Table~\ref{tab:calibration}), because a lapse-independent shift appears
with opposite signs on the causal and acausal branches
(Section~\ref{sec:artifacts}) and every configuration in this ensemble
measures both branches together, so the drift cancels in the estimate.
What the drift does change is the coherence, which raises the
within-method floor of eq.~\ref{eq:weaver}: the fitted rescale falls
from \(s=11.7\) to \(10.9\) while the median \(\sigma_{C_d}\) is
unchanged (\(0.225\) against \(0.222\,\%\)). The cancellation is a
property of the two-branch measurement, not evidence that the ensemble
could have detected it. A seasonal source effect confined to the late
coda, the waveform-level artefact of \citet{Zhan2013}
(Section~\ref{sec:artifacts}), is the genuine shared error: it warps the
coda every configuration measures, and on that scenario the credible
band covers the truth at only 0.34 of the epochs and the RMSE of \(\mu\)
more than doubles, from 0.067 to \(0.161\,\%\), while the member-level
coverage of \(C_d\) stays at 0.95. That last number is right for a
reason that gives no comfort: the artefact also inflates the scatter
that \(s\) absorbs, so \(C_d\) widens, but its coherent part biases
every member and the ensemble mean together and leaves no trace in their
spread. That is the limit of the method, not a defect of the fit: an
ensemble over processing choices can only reveal the errors on which its
members disagree, and an error shared by every pipeline (a source
change, a contaminated reference, a clock drift measured on one branch)
needs an independent diagnostic, such as the branch asymmetry of
...
literature does and does not yet report. The depth stage is implemented
in the open codameter package as a linear inversion that consumes a
per-band covariance; it is not evaluated in this paper
...
posterior covariance \(C_m(z)\); codameter implements this inversion,
with a diagonal or a full band covariance as supplied, and we do not
evaluate it here. The bands must sit where the kernels resolve: too low
and the kernel leaks into the half-space, too high and the coda is
incoherent (Section~\ref{sec:params_freq}). What the stage should report
is the width of \(C_m(z)\) together with the profile, and that width
inherits whatever correlation the supplied per-band covariance carries:
a diagonal covariance gives a width that understates the effect of
errors the bands share.
...
change and, downstream, the stress. Carrying \(\delta\rho/\rho\) as a
second inverted field with its own kernel, so that the inversion reports
how much of the surface \dvv~each field explains, is a natural extension
of the linear stage; codameter does not implement it.
...
series by about 45 days, and comparing without matching that smoothing
caps the correlation near 0.7 even on a real annual cycle
(Fig.~\ref{fig:realdata-validation}). We match by applying the same
trailing 90-day mean to the daily series (at least 45 finite days in the
window), compare demeaned --- the two products reference different
epochs, and a constant offset is bookkeeping, not error --- and exclude
the first 150 days of each station's series as reference burn-in. The
comparison is computed by a script archived with the paper
...
\(0.02\) to \(2.8\,\%\) over the 96 pipelines of the 108-pipeline
factorial that return a series, against \(0.033\,\%\) for the
best-practice baseline, and the reported network error bar changes by a
factor \(\sqrt{N}\) with the quantity a study names. Whether processing
also dominates the variability between real deployments is a question
these experiments cannot answer, because they hold the observed waveform
fixed; the repeated-realisation calibration of Section~\ref{sec:bayes}
redraws only the additive noise on one fixed coda, so it varies the
noise-limited part of the error and not the wavefield, and is a first
step toward that question rather than an answer to it. The problem is
...
physical quantity in the field, so the configuration axis should be
marginalised only over pipelines that target one estimand; the two bands
of our ensemble see one imposed \dvv~on the synthetic, which is why they
can be combined there. And the coherence floor of \citet{Weaver2011} is
a lower bound: on our synthetic the residual scatter about the ensemble
mean is an order of magnitude above it, which the fitted rescale \(s\)
reports rather than hides. The pointwise tests in
Table~\ref{tab:calibration} support the \(95\,\%\) member intervals on
the listed scenarios. They also expose \(68\,\%\) overcoverage and poor
coverage of the combined estimate. They do not establish temporal
...
package is unit-tested. The three real-data figures were produced by the
cloud run and are committed to the repository as produced, not
regenerated (Section~\ref{sec:deployment}). An executable record turns
an undocumented choice into a versioned, inspectable one, and lets a
reader re-run a study's pipeline on the truth-known synthetic to see its
bias before trusting it on data.
...
than two orders of magnitude (0.02 to \(2.8\,\%\)) across the 96
pipelines of a 108-pipeline factorial that return a series, on one
synthetic volcano scenario, and, more consequentially, change the number
reported as its uncertainty by a factor of \(\sim\!\sqrt{N}\) depending
on whether a study calls \(1\sigma\) the standard error of the network
mean or the between-pair standard deviation, all without touching the
data. The remedy is not a single mandated pipeline but transparency:
report every choice and name the quantity each error bar estimates,
sample the choices the physics does not fix, carry the resulting
measurement covariance into the inference after validating what it is
meant to cover, and release the pipeline as executable code. Repeated
synthetic realisations support the proposed \(95\,\%\) pointwise
intervals for the error of a single processing configuration, while the
nominal \(68\,\%\) intervals overcover and the credible band on the
...
Converting a depth profile to stress or strain is a further step this
paper does not attempt. We offer codameter as one such record; its depth
stage is an interface awaiting evaluation.
...
generated figure's sidecar records its generating commit and numerical
arrays. The figures and manuscript can originate from different
revisions. \texttt{python -m codameter.figures} regenerates every
generated figure with a \texttt{.npz} sidecar of its plotted arrays and
a \texttt{.json} sidecar of its provenance;
...
\citet{Clements2023} series archived there. The daily products and the
three real-data figures were produced by the noisepy-dvv-cloud Gate 1
run from public Southern California Earthquake Data Center waveforms;
the products are available locally during this analysis but are not
tracked in the repository. The exact Gate 1 run commit, run
configuration and redistribution archive remain to be supplied; their
current provenance limits are recorded in
