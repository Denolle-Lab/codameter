...
we report over 103 publications in Appendix~\ref{app:survey}. We do not
aim to report the ``best'' pipeline, which is most often the one
reported in scientific papers, but instead document the parameter
impacts (Section~\ref{sec:results}). We then propose a new measurement
error that incorporates these effects into a data covariance matrix
\(C_d\) (Section~\ref{sec:bayes}). Throughout this paper, key notation
includes: \(\sigma\) (measurement uncertainty of a recovered
\dvv~estimate), \(B\) (frequency bandwidth), and \(W=[t_1,t_2]\) (coda
window); subscripts distinguish specific contexts (e.g., \(\sigma_k\)
for per-component uncertainty), each defined where first introduced.
...
is a limit case rather than a practice: we found no surveyed publication
that re-baselines continuously and reports the increments without
summing them. It is retained here because it isolates what re-baselining
costs when the trend is not reconstructed, which is the failure mode the
two families above exist to avoid. codameter implements the joint
inversion but neither the cumulated trailing reference nor cluster-based
reference selection.
...
the 96 publications of the survey that measure \dvv~(of 103 catalogued;
77 from ambient noise alone, 9 from earthquake coda alone, 10 from
both).
...
but Table~\ref{tab:survey} shows many surveyed publications instead
reuse one fixed window across bands.
...
publications, one per publication, each identified by its DOI (by its
ADS record for the one conference abstract, \citet{Kristjansdottir2019};
the machine-readable table and its provenance are distributed with the
...
uncertainty treatment vary from publication to publication, and the last
is frequently unreported. Every publication is cited here.
...
(\texttt{literature/README.md} records the corrections). The table keeps
a publication when it measures a temporal relative velocity change from
the coda of ambient-noise correlations or of repeating earthquakes, or
when it defines a method or a sensitivity kernel that the measuring
publications rely on. Seven of the 103 do not themselves measure
\dvv~(theory, depth kernels, deconvolution interferometry, spectral
methods) and are marked \texttt{n/a} in the estimator column; the counts
below exclude them. Of the 96 that measure \dvv, 77 use ambient noise
alone, 9 use earthquake coda alone and 10 both (the machine-readable
table records this per row); the coda-only rows are kept because their
processing choices are the same choices, made on a different wavefield.
...
the 82 publications we could read (open access plus institutional
access); for those rows a blank or \texttt{n/r} means the value is
genuinely not stated in the paper. The remaining 21 rows were populated
from abstracts and search metadata, where \texttt{n/r} means only that
the value was not found in the abstract, not that the publication failed
to report it; those cells are flagged in the machine-readable table
(\texttt{measurement\_source}), remain to be filled from the paywalled
full texts, and are excluded from the rates below. Among the 75
full-text publications that measure \dvv, 68 report the frequency band,
59 the coda window and 74 the estimator. For the uncertainty column we
distinguish a stated error on \dvv~(a formula, a standard deviation or
error, a confidence interval, a regression or bootstrap error, a stated
noise or significance level) from a quality-control rule alone (a
coherence or correlation threshold, a weighting, a rejection criterion,
with no error magnitude) and from nothing: 57 of the 75 state an error,
11 state a quality rule only, and 7 state neither. The rule, the keyword
coding and the eight cells coded by hand are in
\texttt{paper/survey\_rates.py}, which writes these counts to
