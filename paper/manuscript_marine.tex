...
(standard deviation) of a recovered \dvv~estimate. Subscripts and
modifiers distinguish specific types: \(\sigma_k\) is the per-component
uncertainty, \(\sigma_{\varepsilon}\) is the measurement error on the
raw stretch factor, \(\sigma_{\mathrm{SE}}\) and
\(\sigma_{\mathrm{SD}}\) are the standard error and standard deviation
conventions, and \(\sigma_d\) is the data covariance matrix diagonal;
each is defined in context where it appears.
