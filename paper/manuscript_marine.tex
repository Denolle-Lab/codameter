...
81 from ambient noise alone, 11 from earthquake coda alone, 11 from
both).
...
table and its provenance are distributed with the codameter package). It
is the empirical basis for the paper's claim that no convention is
shared: the estimator, the frequency band, the coda window, the
reference scheme and --- most unevenly of all --- the uncertainty
treatment vary from publication to publication, and the last is
frequently unreported. Every publication is cited here.
...
methods) and are marked ``n/a'\,' in the estimator column; the counts
below exclude them. Of the 96 that measure \dvv, 81 use ambient noise
alone, 9 use earthquake coda alone and the rest both (the
machine-readable table records this per row); the coda-only rows are
kept because their processing choices are the same choices, made on a
different wavefield.
...
access); for those rows a blank or
\texttt{n/r\textquotesingle{}\textquotesingle{}\ means\ the\ value\ is\ genuinely\ not\ stated\ in\ the\ paper.\ The\ remaining\ 21\ rows\ were\ populated\ from\ abstracts\ and\ search\ metadata,\ where}n/r'\,'
means only that the value was not found in the abstract, not that the
publication failed to report it; those cells are flagged in the
machine-readable table (\texttt{measurement\_source}), remain to be
filled from the paywalled full texts, and are excluded from the rates
below. Among the 75 full-text publications that measure \dvv, 68 report
the frequency band, 59 the coda window and 74 the estimator. For the
uncertainty column we distinguish a stated error on \dvv~(a formula, a
standard deviation or error, a confidence interval, a regression or
...
nothing: 57 of the 75 state an error, 11 state a quality rule only, and
7 state neither. The rule, the keyword coding and the eight cells coded
by hand are in \texttt{paper/survey\_rates.py}, which writes these
counts to \texttt{paper/data/survey\_rates.json}.
