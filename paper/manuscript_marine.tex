...
its stated uncertainty has not been quantified in the literature.
...
(Sections\textasciitilde{}\ref{sec:depth}) consumes.

Each subsection below is written to serve three readers at once: a
seismologist checking the mechanism against a realistic synthetic, a
geophysicist who wants the scale of the resulting error without
re-deriving it, and an automated agent or pipeline that needs a
concrete, quotable rule for setting the same parameter. The closing
\textbf{Scale of effect} sentence in each subsection is written for that
last use directly.

Each of the parametric components is implemented in a single package,
\texttt{codameter}, that borrows from \texttt{msnoise}
\citep{Lecocq2014} and \texttt{noisepy} \citep{Jiang2020} to extract the
measurement of \dvv~from the ambient noise monitoring workflow and
generalize it to any coda wave from repeated source-receiver paths.
