...
The temporal resolution of ambient-noise \dvv~measurements is
fundamentally limited by the tradeoff between the duration required for
stable noise correlations and the need to resolve rapid medium changes;
however, filtering, denoising, and exploitation of spatial or temporal
redundancy can improve the signal-to-noise ratio of short-time
correlations and thereby permit stable \dvv~estimates at increasingly
fine temporal resolution
\citep{Hadziioannou2011,Moreau2017,Mao2019,Viens2020}.
...
or even other forms of denoising (), and how to aggregate and weight the
many cross-component and station-pair measurements that make up a single
reported \dvv~time series (e.g., \citet{Hobiger2012}). These choices are
made by habit, justified briefly if at all, and rarely reported in
enough detail to reproduce. The community has long flagged
\emph{individual} pitfalls (spurious changes from non-stationary noise,
\citet{Zhan2013}; measurement-error formulae, \citeauthor{Clarke2011}
...
structural, not a noise effect that more averaging fixes. Field
deployments sometimes need a multiple-reference approach for the same
underlying reason, when no single period is long or quiet enough to
serve as a stable baseline (e.g., the urban Mexico City deployment of
\citet{Ermert2023}); our moving-reference sweep is the closest
single-parameter analog available in codameter today, not a reproduction
of any one study's exact reference construction.
...
\citep{Hadziioannou2011,Moreau2017,Mao2019,Viens2020}. In this paper,
the choice of substack length is guided by data-dependent quality gates
(the CC threshold above) rather than a fixed duration, which allows
stations with high coherence to preserve shorter temporal windows and
thereby track rapid changes, while stations with lower SNR substack as
needed to achieve stable estimates.
