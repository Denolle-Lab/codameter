...
time-dependent single-measurement covariance \(C_d\). On 20 independent
synthetic realisations of a volcano scenario, \(C_d\) covers 95.5,\% of
...
the truth lies within \(1.96\,\sigma_{C_d}\) is 0.955, within the margin
of \(\pm 0.03\) fixed before the run: \(C_d\) is calibrated as the
covariance of a single measurement at the 95,\% level. At the 68,\%
level it covers 0.80, so the member residuals are more peaked than a
Gaussian. The 95,\% credible band on \(\mu\) contains the truth at only
0.59 of the epochs: the ensemble mean inherits the biases its members
share (the stacking that rounds the drop, the estimator biases at low
coherence), and no amount of averaging over configurations removes them,
which is what the posterior's precision cannot know. An earlier draft of
this paper compared \(\mu\) with \(\pm 2\sigma_{C_d}\) and read the
resulting 100,\% coverage as nominal; that comparison mixes the ensemble
mean with a single-measurement scale and over-covers by construction,
and we no longer make it. The other two scenarios of
Table~\ref{tab:calibration} inject artefacts that every configuration
sees identically. A station clock drift leaves every statistic
unchanged, because a lapse-independent shift appears with opposite signs
on the causal and acausal branches (Section~\ref{sec:artifacts}) and
every configuration in this ensemble measures both branches together, so
the drift cancels in the estimate; that is a property of the two-branch
measurement, not evidence that the ensemble could have detected it. A
seasonal source effect confined to the late coda, the waveform-level
artefact of \citet{Zhan2013} (Section~\ref{sec:artifacts}), is the
genuine shared error: it warps the coda every configuration measures,
and on that scenario the credible band covers the truth at only 0.35 of
the epochs and the RMSE of \(\mu\) doubles, from 0.070 to 0.159,\%,
