...
and station pairs are aggregated and weighted, made ad hoc and reported
incompletely, so the same data can support different values and
different error bars. We quantify the individual and combined effects of
these choices on synthetic correlation functions in which the
...
they induce. We then propose a hierarchical Bayesian measurement model
that runs an ensemble of defensible pipelines on the same correlations
and marginalises the processing choice into a single \dvv~series and a
time-dependent single-measurement covariance \(C_d\). On 200 independent
synthetic realisations of a volcano scenario, \(C_d\) covers 95.6,\% of
individual member errors at the 95,\% level, whereas the credible band
on the ensemble mean covers the truth only 59,\% of the time because the
configurations share time-varying biases; a seasonal source artefact
seen by every configuration halves that coverage again and doubles the
error of the ensemble mean while leaving \(C_d\) nominally calibrated,
because an ensemble over processing choices can only see the errors on
which its members disagree. We set out how \(C_d\) enters a depth
...
run on NoisePy correlations in a cloud batch pipeline, reproduces a
published \dvv~product (correlation 0.83--0.99 on 360--580 matched days,
with a reproducible comparison script), a comparison that also exposed
and corrected a sign-convention error. The framework, estimators,
figures with their numerical sidecars, and the calibration experiments
are released in the open Python package \texttt{codameter}, together
with a processing-choice advisor and a seeded golden dataset whose
scorer cannot be satisfied by sparse predictions; an evaluation of
...
on NoisePy correlations for three California stations, with a published
product (Section~\ref{sec:deployment}). We package the methodology in a
Python software, \texttt{codameter}, which also ships an advisor skill:
an AI agent can be asked to recommend a processing configuration or
...
and dataset are part of the release; an evaluation of agents against
them is not part of this paper.
...
On small, clean \dvv\(\,\)all seven estimators agree to within 0.04,\%
RMS (Fig.~\ref{fig:methods}a). Sweeping the same clean recovery out to
\(\pm 5\,\%\) (Fig.~\ref{fig:methods}b) shows exactly where and how each
family first departs from the 1:1 line, and the estimator choice becomes
consequential at large, noisy \dvv~(Fig.~\ref{fig:methods}c), where the
effect of the methods is split according to their phase measurement
approaches. The stretching family (TS, WTS) and WCC match the whole
dilated coda and stay within 0.05,\% RMS of the truth on the noisy
landslide signal (0.005, 0.010 and 0.046,\%); the warping methods (DTW,
WTDTW) track but under-shoot the largest strains (0.16 and 0.08,\% RMS);
both phase methods fail on this noisy, large-strain signal, MWCS by
cycle-skipping (2.9,\% RMS) and WCS, whose 2-D unwrapping recovers the
clean sweep of panel (b), by losing the phase track once noise is added
(3.8,\% RMS; the numbers are computed from the figure's numerical
sidecar). No estimator is simply ``right''; the choice of estimator
materially changes the \dvv~measurement at larger strain.
...
(\(\sim\!0.067\,\%\), down from \(\sim\!0.71\,\%\) at 1 day): the noise
...
and frequency band deviations are each more modest
(\(1.5\)--\(2.5\times\)). The joint-inversion reference stays closest to
the fixed-reference baseline among the reference-scheme deviations.
...
\label{eq:bayes}
\end{equation} with a second-difference smoothness prior of precision
...
scales are small against the data terms; codameter reports the share of
each posterior that the prior supplies, which on the synthetic below is
a few percent for \(\tau^2\) and negligible for \(s^2\) and \(\lambda\)
(Table~\ref{tab:calibration}). A conjugate Gibbs sampler, implemented in
codameter with no external sampler dependency and with a banded solve
for the smoothness update, returns the joint posterior.
...
and shrinks with ensemble size. The object a downstream depth or stress
inversion must consume when it takes a single \dvv~series as data is the
\textbf{single-measurement covariance} \begin{equation}
...
model, not sampled; it is the covariance of one measurement drawn from
the ensemble, not of \(\mu\).
...
they usually do (Fig.~\ref{fig:bayes}c). The temporal correlation
(\(L\approx 40\) days at a 4-day cadence) and the common-mode term
collapse the effective number of independent epochs from 229 to a few
tens.
...
\(C_d\) is calibrated as the covariance of a single measurement at the
95,\% level. At the 68,\% level it covers 0.81, so the member residuals
are more peaked than a Gaussian. The 95,\% credible band on \(\mu\)
...
precision cannot know. An earlier draft of this paper compared \(\mu\)
with \(\pm 2\sigma_{C_d}\) and read the resulting 100,\% coverage as
nominal; that comparison mixes the ensemble mean with a
single-measurement scale and over-covers by construction, and we no
longer make it. The other two scenarios of Table~\ref{tab:calibration}
inject artefacts that every configuration sees identically. A station
clock drift leaves every statistic unchanged, because a
...
in the estimate; that is a property of the two-branch measurement, not
evidence that the ensemble could have detected it. A seasonal source
effect confined to the late coda, the waveform-level artefact of
...
and the RMSE of \(\mu\) more than doubles, from 0.067 to 0.161,\%, while
the member-level coverage of \(C_d\) stays at 0.95. That last number is
right for a reason that gives no comfort: the artefact also inflates the
scatter that \(s\) absorbs, so \(C_d\) widens, but its coherent part
biases every member and the ensemble mean together and leaves no trace
in their spread. That is the limit of the method, not a defect of the
fit: an ensemble over processing choices can only reveal the errors on
which its members disagree, and an error shared by every pipeline (a
source change, a contaminated reference, a clock drift measured on one
branch) needs an independent diagnostic, such as the branch asymmetry of
...
measurement covariance from Section~\ref{sec:bayes}. Stacking bands
...
is incoherent (Section~\ref{sec:params_freq}). The width of \(C_m(z)\)
...
reasonable processing choices through the same correlations yields, by
the law of total variance, a marginal \dvv~uncertainty that includes the
processing-choice spread, and Section~\ref{sec:bayes} makes this a
Bayesian measurement model whose deliverable is a time-dependent
single-measurement covariance \(C_d\). Its limitations are as important
as its construction. The prior over pipelines is a menu, not a
representative sample of practice, and the covariance is conditional on
it. Every configuration transforms the same waveforms, so an error they
all share (a source change, a contaminated reference, a clock drift
measured on one branch) leaves no trace in their spread: on the
shared-source scenario of Table~\ref{tab:calibration} the ensemble
agrees with itself, the error of its mean doubles, and \(C_d\) cannot
see it. Configurations in different bands sample different depths and
may not estimate the same physical quantity, so the configuration axis
should be marginalised only over pipelines that target one estimand. And
the coherence floor of \citet{Weaver2011} is a lower bound: on our
synthetic the residual scatter about the ensemble mean is an order of
magnitude above it, which the fitted rescale \(s\) reports rather than
hides. An error bar built this way is more complete than any single
pipeline's, not automatically calibrated; Table~\ref{tab:calibration} is
what makes it a tested claim on the scenarios listed there.
...
cloud run and are archived, not regenerated
...
with seasonal hydrology at three stations (\(r=-0.69,-0.45,-0.40\) under
the rule used at the time, recorded in the codameter v0.4.0 release
notes). Ground-truthing through the exact call path made the cause
obvious: imposing a \(+0.5\,\%\) velocity change returned \(-0.50\,\%\).
The synthetic generator and all seven estimators had consistently used
the stretch factor \(\varepsilon\) (positive for a coda dilation, i.e.~a
slowdown), not physical \dvv~(positive for a speedup) --- internally
coherent, so every synthetic-recovery test in the sections above passed,
but opposite to the sign convention the field expects and to the
published product it was compared against. The fix is the convention
boxed in Section~\ref{sec:intro}, shipped as codameter v0.4.0. Internal
...
that these choices change the recovered \dvv~by up to two orders of
magnitude in RMS across reasonable pipelines and, more consequentially,
change the number reported as its uncertainty by a factor of
\(\sim\!\sqrt{N}\) depending on which of two different quantities a
study calls \(1\sigma\), all without touching the data. The remedy is
not a single mandated pipeline but transparency: report every choice and
name the quantity each error bar estimates, sample the choices the
physics does not fix, carry the resulting measurement covariance into
the inference, and release the pipeline as executable code. The
covariance we propose is calibrated against member errors on repeated
synthetic realisations and blind, by construction, to errors every
configuration shares (Table~\ref{tab:calibration}); it is the input a
depth inversion consumes, and carrying it that far is necessary, not
sufficient, for an honest depth-resolved uncertainty, which also needs
the kernels' own uncertainty. Converting a depth profile to stress or
strain is a further step this paper does not attempt. We offer codameter
as one such record; its depth stage is an interface awaiting evaluation.
...
boxed in Section~\ref{sec:intro}, \(\dvv=-\varepsilon/(1+\varepsilon)\).
...
\(\delta t_i=+\varepsilon\,t_i\); with that sign every estimator below
returns the dilation \(\varepsilon\) of the Introduction, and a
regression test in codameter holds each of them to it in both signs.
...
\([f_1,f_2]\) yields \(\delta t_i\), and \(\varepsilon\) is the slope of
\(\delta t_i\) versus \(t_i\) \citep{Clarke2011}. Because \(\varphi\) is
defined modulo \(2\pi\), the estimate cycle-skips once
\(|\delta t_i|>1/(2f)\).
...
of change); \(\varepsilon\) is the slope of the lag \(l(i)/f_s\)
(current relative to reference) versus lapse.
...
ambient-noise \dvv~monitoring studies we surveyed: 103 rows for 102
publications, one publication \citep{Obermann2013} contributing two rows
for two distinct measurement set-ups (the full machine-readable version
and provenance are distributed with the codameter package). It is the
...
openly available at \url{https://github.com/Denolle-Lab/codameter}; the
manuscript is built from the commit recorded in each figure's sidecar.
\texttt{python -m codameter.figures} regenerates every generated figure
with a \texttt{.npz} sidecar of its plotted arrays and a \texttt{.json}
sidecar of its provenance; \texttt{python -m codameter.calibration}
reproduces Table~\ref{tab:calibration}
...
archived there. The daily products and the three real-data figures were
produced by the noisepy-dvv-cloud Gate 1 run from public Southern
California Earthquake Data Center waveforms; the products are archived
with the paper and the run's commit is recorded in
\texttt{paper/data/gate1/README.md}. A versioned archive with a Zenodo
DOI will be issued alongside publication.
...
through one review iteration prior to submission; all findings were
reviewed and adjudicated by the author, and the tool does not endorse
the manuscript's validity. Code revisions and text edits in response to
that review were drafted with Claude (Anthropic) under the author's
direction and are recorded in the repository history.
