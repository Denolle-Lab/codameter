...
covariance \(C_d\) --- the object a downstream depth or stress inversion
needs. We argue that these undocumented choices, not the physics, are
...
Fournaise (\citet{Brenguier2008}) and is now calculated in continuous
along side of more conventional seismic monitoring methods in the same
volcano observatory (\citet{Duputel2009}) and was a determining early
...
embankment integrity (Planès et al., 2016), and geothermal and CO₂
...
well established (\citet{Obermann2013}). We package this new methodology
...
often assumed in practce that coda waves of cross-components with
...
them directly (REF), or weighted (e.g., weights of their ) How they are
combined is a workflow choice that is almost never reported, yet it
changes both the value and the uncertainty
...
unambiguous: relative to a best-practice RMS error of \$\sim\$0.02,\%,
an unwrapped phase estimator that cycle-skips is catastrophic, a moving
...
precision here) and the gating are second-order. The Brenguier joint
inversion is nearly as good as the fixed reference and preserves the
trend.
